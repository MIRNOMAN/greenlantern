...
const totalPending = totallData.filter((pharmacist: PharmacyInfo) => pharmacist.status === "PENDING").length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
     {isLoading ? ( <div>Loading</div>) : ( <div className=" mx-auto space-y-6">
        ...
        <div className="bg-white rounded-lg p-4 shadow-sm">
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <p className="text-lg md:text-[20px] leading-[28px] font-semibold text-gray-600">Pending Requests</p>
              <h3 className="text-xl md:text-[40px] leading-[48px] font-semibold mt-1">{totalPending}</h3>
            </div>
          </div>
        </div>
                  ...
                  <tr key={pharmacist.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-[18px] leading-[28px] font-medium">{pharmacist.user.firstName +
                              " " +
                              pharmacist.user.lastName}</div>
                          <div className="text-[16px] leading-[26px] font-normal text-[#515050] opacity-80">{pharmacist.user.email}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(pharmacist.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
